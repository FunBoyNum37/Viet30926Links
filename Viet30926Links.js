...
<body>
    <div class="container">
        <div class="header">
            <div class="logo"><img src="https://cloud.viet30926.qzz.io/file/logo/1778858808206_Logo_-_NoBack.png" alt="Viet30926Links" onerror="this.onerror=null;this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22120%22 height=%22120%22%3E%3Ctext x=%2250%25%22 y=%2250%25%22 font-size=%2248%22 text-anchor=%22middle%22 dominant-baseline=%22middle%22%3E%F0%9F%9A%80%3C/text%3E%3C/svg%3E'"></div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Quản lý Links - Viet30926Links</title>
    <style>
        * {margin:0;padding:0;box-sizing:border-box}
...
<body>
    <div class="navbar">
        <div class="navbar-brand"><img src="https://cloud.viet30926.qzz.io/file/logo/1778858808206_Logo_-_NoBack.png" alt="Logo" style="max-width:30px"> Viet30926Links - Quản Lý</div>
    ...
    <script>
        ...
        function renderLinks() {
            ...
            tbody.innerHTML = allLinks.map(function(link) {
              ...
              return '<tr>' +
                ...
                '<td><div class="actions">' +
                  '<button class="action-btn copy-btn" onclick="copyLink(' + JSON.stringify(link.shortUrl) + ', this)">📋 Copy</button>' +
                  '<button class="action-btn delete-btn" onclick="deleteLink(' + JSON.stringify(link.slug) + ')">🗑️ Xóa</button>' +
                '</div></td>' +
                '</tr>';
            }).join('');
        }
...
<body>
    <div class="container">
        <div style="text-align:center;margin-bottom:20px">
            <img src="https://cloud.viet30926.qzz.io/file/logo/1778858808206_Logo_-_NoBack.png" alt="Logo" style="max-width:80px;height:auto" onerror="this.onerror=null;this.style.display='none'">
        </div>
        <div class="emoji">😕</div>
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Route: GET /links - Links Management Page
      if (pathname === '/links' && request.method === 'GET') {
        return new Response(generateLinksPage(), {
          headers: { 'Content-Type': 'text/html; charset=utf-8' },
          status: 200
        });
      }
      ...
      if (pathname === '/api/links' && request.method === 'GET') {
        ...
        try {
          ...
          for (const key of listResult.keys) {
            ...
            const linkDataStr = await env.VIET30926_DB.get(key.name);
            if (linkDataStr) {
              try {
                const linkData = JSON.parse(linkDataStr);
                links.push({
                  slug: key.name,
                  shortUrl: `https://${CUSTOM_DOMAIN}/${key.name}`,
